...
_hist_picker() {
    ...
    # --accept-nth 2 returns it verbatim. change:first — typing resets
    # the highlight to the topmost (newest) match; without it fzf keeps
    # the cursor index where Up/Down left it, so a new query silently
    # skips the matches above that line.

...
# Tabular transform: rows are "command\x1f meta\x1fid" — rebuilt as
# "display\x1f full \x1f meta \x1f id". display = the command padded or
...
# gets accepted — matching (--nth 1,2) sees display + full, so text
# beyond the truncation point still matches, while the meta (field 3)
# stays out of the match. Multi-line commands keep their native display
# (truncating them would destroy their structure). gawk under a UTF-8
# locale counts characters (correct column width); mawk or a C locale
# counts bytes — truncation still works, the column drifts only on
# multibyte commands.
...
BEGIN { RS = "\0" }
{
    row = $0
    if (row == "") next
    i = index(row, "\037")            # command \x1f meta \x1f id
    ...
    printf "%s\037%s\037%s\037%s\036", disp, full, meta, id
}' | tr '\036' '\000'
